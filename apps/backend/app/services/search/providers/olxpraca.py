...
def _build_search_url(keywords: list[str] | None) -> str:
    slug = _slugify_query(_build_query(keywords))
    return f"{OLX_BASE_URL}/praca/q-{slug}/"
...
def _extract_offer_urls(html_text: str) -> list[str]:
    ...
    for match in re.finditer(r"""href=['"]([^'"]+)['"]""", html_text, flags=re.IGNORECASE):
        ...
        if "/oferta/praca/" not in candidate:
            continue
        if candidate.startswith(("ios-app://", "android-app://")):
            continue
        if candidate.startswith("/"):
            candidate = f"{OLX_BASE_URL}{candidate}"
...
def _normalize_offer(url: str, html_text: str, index: int) -> ScrapedOffer | None:
    ...
    return ScrapedOffer(
        id=_offer_id_from_url(normalized_url, f"olxpraca-{index}"),
        source="olxpraca",
        title=title,
        company=company,
        location=location,
...
async def scrape_olxpraca(
    target_count: int | None,
    on_progress: ProgressHandler | None = None,
    *,
    keywords: list[str] | None = None,
) -> list[ScrapedOffer]:
    """Scrape OLX Praca offers for the current keyword query."""
    response = await fetch_with_timeout(
    ...
    if response.status < 200 or response.status >= 300:
        raise RuntimeError(f"OLX Praca request failed with status {response.status}")
